...
test.describe('TeaCup Form Example App', () => {
  ...
  test('1. should show the main heading and all field labels', async ({ page }) => {
  ...
  test('3. should add and remove tags (TextPill)', async ({ page }) => {
    ...
    // Remove tag
    await page.locator('button').filter({ hasText: 'react' }).locator('svg').click();
    await expect(page.getByText('react')).not.toBeVisible();
    await expect(page.getByText('typescript')).toBeVisible();
  });

  test('4. should show validation for empty tags on submit attempt', async ({ page }) => {
  ...
  test('5. should select an option from the dropdown', async ({ page }) => {
    ...
    await expect(page.getByText('Cambodia')).toBeVisible();
  });
  ...
  test('6. should select a date in the calendar', async ({ page }) => {
    ...
    const value = await page.getByPlaceholder('Select a date').inputValue();
    expect(value).not.toBe('');
  });

  test('7. should handle file uploads and preview', async ({ page }) => {
    const fileChooserPromise = page.waitForEvent('filechooser');
    await page.getByText('Click or drop files here').click();
  ...
  test('8. should remove an uploaded file', async ({ page }) => {
    ...
    await page.locator('button').filter({ has: page.locator('svg line') }).last().click();
    await expect(page.getByText('delete-me.txt')).not.toBeVisible();
  });

  test('9. should trigger all validations on invalid submit attempt', async ({ page }) => {
    const submitBtn = page.getByRole('button', { name: 'Submit Form' });
    await submitBtn.click();
  ...
  test('10. should submit the form successfully when all fields are valid', async ({ page }) => {
    // Fill Username
